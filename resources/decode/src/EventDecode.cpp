...
//szAnalysisFolder=pwd+pCacheFolder，并创建这个路径。
//把pSourceZip文件解压，放到szAnalysisFolder下。
//在解压目录下根据文件名的尾部标识，分类分别把文件名存入unZipFiles。
bool _gLog_DecompZIP(UNZIP_ALL_FILE& unZipFiles, char* szAnalysisFolder, char* pCacheFolder, const char* pSourceZip)
{
	...
	if (sfl_getcwd(szAnalysis, sizeof(szAnalysis)))
	{
		...
		else
		{
			...
			sfl_mkdir(szAnalysis, 0777);
			WriteLog(LOG_LEVEL_ERROR,"CEventDecode::_gLog_DecompZIP","already exist:%s",szAnalysis);
		}
		if ( 0 == sfl_access(szAnalysis, MODE_WRITE_PERMISSION) )
		{
			WriteLog(LOG_LEVEL_DEBUG,"CEventDecode::_gLog_DecompZIP","tmp_dir:%s",szAnalysis);
			strcat(szAnalysis,  LOG_SEPERATOR_DIR);
			//in a situation 'sfl_unzip()' unzip files successfully but the return value is false
			//so cancel check the return value
			sfl_unzip(pSourceZip, szAnalysis);
			...
			//{
				...
				do
				{
					...
					{
						//log
						WriteLog(LOG_LEVEL_DEBUG,"CEventDecode::_gLog_DecompZIP","file %s",pfile);
						char Suffix[5] = {0};
						...
						if (nLen> 4)
						{
							...
							}else if (strcmp(Suffix, ".xml") == 0)
							{
								...
								strXML += szAnalysis;
								strXML += pfile;
								unZipFiles.strListXMLFile.push_back(strXML);
							}else
							{
								string strFile;
								strFile += szAnalysis;
				...
				//update 131216 wangyang
				//sfl_unzip() return value maybe not correct,so cancel check sfl_unzip() return value.
				//check if '*.log' is exist,if not,return false.
				if(strlen(unZipFiles.szLogFile))
				{
					return true;
				}
			//}
		...
		{
			WriteLog(LOG_LEVEL_ERROR,"CEventDecode::_gLog_DecompZIP","%s cannot access",szAnalysis);
		}
	}
	return false;
}
...
//add 131211 wangyang
bool PreDecodeEx(PreDecodeCfg &cfg)
{
        CEventDecode logDecode;
        int iRet = logDecode.GetLogInfo(cfg.str_log_path.c_str());
        if (E_RESULT_OK != iRet)
        {
                char szReason[510] = {0};
                switch(iRet)
                {
                case E_RESULT_FILE_NOT_EXISTING:
                        strcpy(szReason, "The file name is null");
                        break;
                case E_RESULT_FILE_CAN_NOT_OPEN:
                        strcpy(szReason, "Open file fail");
                        break;
                case E_RESULT_FILE_INVALID:
                case E_RESULT_FILE_NULL:
                        strcpy(szReason, "Invalid file");
                        break;
                case E_RESULT_FILE_NO_NET:
                        strcpy(szReason, "NO wireless network parameters");
                        break;
                case E_RESULT_MEMORY_FAILURE:
                        strcpy(szReason, "NO memory");
                        break;
                }
                WriteLog(LOG_LEVEL_ERROR, "PreDecode", "m_eventLogDecode.LoadLogFile:%s (%s)(%d)\n", cfg.str_log_path.c_str(), szReason, iRet);
                return false;
        }
        T_LOG_INFO* pLogInfo = logDecode.GetCurLogInfo();

        if (pLogInfo->iLogServiceType == EM_LOG_SERVICE_complaint)
        {
                cfg.str_service = "complaint";
                cfg.str_unhandled_path=SERVICE_COMPLAINT;
        }
        else if ((pLogInfo->iLogServiceType == EM_LOG_SERVICE_inspection)||(pLogInfo->iLogServiceType == EM_LOG_SERVICE_GD_inspection))
        {
                cfg.str_service = "inspection";
                cfg.str_unhandled_path=SERVICE_NORMAL;
        }
        else if ((pLogInfo->iLogServiceType == EM_LOG_SERVICE_spotcheck)||(pLogInfo->iLogServiceType == EM_LOG_SERVICE_GD_spotcheck))
        {
                cfg.str_service = "spotcheck";
                cfg.str_unhandled_path=SERVICE_NORMAL;
        }
        else
        {
                cfg.str_service = "normal";
                cfg.str_unhandled_path=SERVICE_NORMAL;
        }
        if (pLogInfo->bIsIndoor)
        {       
                cfg.str_scene = "indoor";
        }else
        {       
                cfg.str_scene = "outdoor";
        }
        cfg.db_lat = pLogInfo->fLat;
        cfg.db_lon = pLogInfo->fLon;
        ...
        t.tm_min  = usM;
        t.tm_sec  = usS;
        cfg.s32_start_time = mktime(&t);
        
        LON_LAT stPt;
        stPt.dLat = pLogInfo->fLat;
        stPt.dLon = pLogInfo->fLon;
        string strValue;
        if ((pLogInfo->iLogServiceType == EM_LOG_SERVICE_GD_inspection) || (pLogInfo->iLogServiceType == EM_LOG_SERVICE_GD_spotcheck))      
        {
        }else
        {       
                ...
                {       
                        u32 iProvinceID = 0;
                        pGisChecker->Check(stPt, iProvinceID, pLogInfo->iAreaID, strValue);
                        pLogInfo->iCityID = pLogInfo->iAreaID/100;
                        cfg.u32_p_id = iProvinceID;
                        cfg.u32_c_id = pLogInfo->iCityID;
                        if (iProvinceID != 0)
                        {       
                                pLogInfo->iProvinceID = iProvinceID;
                        }
                }
        ...
        cfg.u32_p_id = pLogInfo->iProvinceID;
        cfg.u32_c_id = pLogInfo->iCityID;
        pGisChecker->GetProvinceStrByID(cfg.u32_p_id, cfg.str_province);
        return true;
}
//add end
...
bool PreDecode(std::string str_log_path, std::string& str_service, std::string& str_scene, std::string& str_province, unsigned int& u32_p_id , unsigned int& u32_c_id, double& db_lat, double& db_lon, int& s32_start_tiem)
{
	...
	int iRet = logDecode.GetLogInfo(str_log_path.c_str());
 	if (E_RESULT_OK != iRet)
 	{
		...
		WriteLog(LOG_LEVEL_ERROR, "PreDecode", "m_eventLogDecode.LoadLogFile:%s (%s)(%d)\n", str_log_path.c_str(), szReason, iRet);
 		return false;
 	}
	T_LOG_INFO* pLogInfo = logDecode.GetCurLogInfo();
 	if ((pLogInfo->iLogServiceType == EM_LOG_SERVICE_inspection)||(pLogInfo->iLogServiceType == EM_LOG_SERVICE_GD_inspection))
 	{
...
CEventDecode::CEventDecode()
{
    ...
    T_PARAM_LIST gParamList[E_PARAM_MAX] = {
		...
		{0,0,"CI",  E_PARAM_GSM_CI, RANGE_GSM_CI_MIN,RANGE_GSM_CI_MAX,0, ID_GSM_CI,E_PARAM_TYPE_INT,0, 0, NULL},
		...
		{0,0,"LAC", E_PARAM_GSM_LAC,RANGE_GSM_LAC_MIN,RANGE_GSM_LAC_MAX,0,ID_GSM_LAC,E_PARAM_TYPE_INT,0, 0, NULL},
		...
		{0,0,"CI",E_PARAM_W_SERV_CI,0,0xfffffff,0,ID_WCDMA_CI,E_PARAM_TYPE_INT,0, 0, NULL},
		...
		{0,0,"LAC",E_PARAM_W_SERV_LAC,0,65535,0,ID_WCDMA_LAC,E_PARAM_TYPE_INT,0, 0, NULL},
...
int CEventDecode::LoadLogFile(const char *pSrcFileName)
{
	...
	{
		//log 
		//WriteLog(LOG_LEVEL_ERROR,"CEventDecode::LoadLogFile","GetLogInfo fail,return:%d",iResult);
		return iResult;
	}
	...
	string strValue;
	if ((m_gLogInfo.iLogServiceType == EM_LOG_SERVICE_GD_inspection) || (m_gLogInfo.iLogServiceType == EM_LOG_SERVICE_GD_spotcheck))
	{
	}else
	{
		if ( pGisChecker && (stPt.dLat>0.1) && (stPt.dLon>0.1))
		{
			u32 iProvinceID = 0;
			u32 iAreaID = 0;
			pGisChecker->Check(stPt, iProvinceID, iAreaID, strValue);
			WriteLog(LOG_LEVEL_DEBUG, "CEventDecode::LoadLogFile", "provinceid=%d,areaid=%d", iProvinceID,iAreaID);
			if (iProvinceID != 0)
			{
				m_gLogInfo.iProvinceID = iProvinceID;
			}
			if(iAreaID != 0)
			{
				m_gLogInfo.iAreaID = iAreaID;
			}
		}
	}
	string strMD5Value;
...
#define MAX_PACKET_LEN 2048
	...
	do
	{
		...
		if(	TEVENT_PACKET_HEADER == pPacket->PackageFlag )
		{
			...
			//add 131219 wangyang
			else if(TEVENT_ID_H_PRODUCT == pPacket->EventID) 
			{
				unsigned char * pFF = packetBuf;
				pFF = pFF + strlen((const char*)pFF) + 1;
				strcpy(m_gLogInfo.szLog_Version , ((TEVENT_H_FF_STR*)pFF)->Fileformatversion);
			}
			//add end
			...
			}else if (TEVENT_ID_H_DN == pPacket->EventID)
			{
				...
				//add 131225 wangyang
				std::string strBrand ;
				std::string strBuildVersionStr;
				size_t ipos = strDNString.find_first_of(',');
				if(std::string::npos != ipos)
				{
					//is complaint log
					//DeviceName = brand + "," + buildVersionStr
					strBrand = strDNString.substr(0,ipos);
					strBuildVersionStr = strDNString.substr(ipos+1);
				}
				else
				{
					//is not complaint log
					strBrand = "";
					strBuildVersionStr = strDNString;
				}
				strcpy(m_gLogInfo.szBrand, strBrand.c_str());
				strcpy(m_gLogInfo.szDeviceName, strBuildVersionStr.c_str());
				//add end
			...
			}else if (TEVENT_ID_H_ST == pPacket->EventID)
			{
				...
				}else if (strSTString.compare("DWSurvey") == 0)
				{
					m_gLogInfo.iLogServiceType = EM_LOG_SERVICE_GD_spotcheck;
				}else if (strSTString.compare("DWInspection") == 0)
				{
					m_gLogInfo.iLogServiceType = EM_LOG_SERVICE_GD_inspection;
				}
				else
				{
				...
				)
			{
				...
				( TEVENT_ID_USERINFORATE == pPacket->EventID)||
				( TEVENT_ID_OFF_NET == pPacket->EventID)||
				( TEVENT_ID_USERINFORATE_APP == pPacket->EventID)||
				( TEVENT_ID_APPINFO_V2 == pPacket->EventID)||
				( TEVENT_ID_HTTP_URL_V2 == pPacket->EventID)||
				( TEVENT_ID_APP_OPEN_STATS_V2 == pPacket->EventID)
				)
			{
	...
	bool bIsJudgeNet = true;
	if ((EM_LOG_SERVICE_GD_spotcheck == m_gLogInfo.iLogServiceType) || (EM_LOG_SERVICE_GD_inspection ==m_gLogInfo.iLogServiceType))
	{
		bIsJudgeNet = false;
	}
	if (bIsJudgeNet)
...
void CEventDecode::PushParam_UserAppRate(unsigned int nIndex, unsigned int nTime, unsigned char *pData)
{
	if( pData == NULL)
	{
		return;
	}
	TEVENT_USERINFORATE_APP_STR* pUserInfoRate = (TEVENT_USERINFORATE_APP_STR*)pData;
	TEVENT_USERINFORATE_STR* pUserRate = &(pUserInfoRate->user_rate_info);
	T_PARAM_EVENT_ITEM ei;
	InitEventItem(&ei, nIndex, nTime, TEVENT_ID_USERINFORATE_APP, 0, 0);
	ei.nParam1 = GetU32(pUserRate->nStartTime);
	ei.nParam2 = GetU32(pUserRate->nEndTime);
	ei.nParam3 = GetU32(pUserRate->nTotaDataBytes);
	ei.nParam4 = GetU32(pUserRate->nDruationTime);
	ei.nSystem = pUserRate->nSystem;

	double fLat = GetFLOAT(pUserRate->fLat);
	double fLon = GetFLOAT(pUserRate->fLon);
	if (g_psTranslate->TransLonLat(fLat, fLon))
	{
		ei.iParam5 = fLat*1000000;
		ei.iParam6 = fLon*1000000;
	}else
	{
		ei.iParam5 = GetFLOAT(pUserRate->fLat)*1000000;
		ei.iParam6 = GetFLOAT(pUserRate->fLon)*1000000;
	}
	ei.iParam7   = GetU16(pUserRate->ucMCC);
	ei.iParam8   = pUserRate->ucMNC;
	ei.iParam9   = GetU16(pUserRate->usCi);
	ei.iParam10  = GetU16(pUserRate->usLac);
	int len_id   = GetU16(pUserInfoRate->length_app_id);
	int len_name = GetU16(pUserInfoRate->length_app_name); 
	if(len_id >= sizeof(ei.szEventName))
	{
		return;
	}
	memcpy(ei.szEventName, pUserInfoRate->app, len_id);
	wchar_t szBuffer[260] = {0};
	int nIndex_1 = 0;
	int nLen     = len_name;
	for (int n=0; n<nLen; )
	{
		memcpy(&(szBuffer[nIndex_1]), &((pUserInfoRate->app + len_id)[n]), 2);
		n += 2;
		nIndex_1++;
	}
...
void CEventDecode::PushParam_UserRate(unsigned int nIndex, unsigned int nTime, unsigned char *pData)
{
	if(NULL == pData)
	{
		return;
	}
	TEVENT_USERINFORATE_STR* pUserRate = (TEVENT_USERINFORATE_STR*)pData;
	T_PARAM_EVENT_ITEM ei;
	InitEventItem(&ei, nIndex, nTime, TEVENT_ID_USERINFORATE, 0, 0);
	ei.nParam1 = GetU32(pUserRate->nStartTime);
	ei.nParam2 = GetU32(pUserRate->nEndTime);
	ei.nParam3 = GetU32(pUserRate->nTotaDataBytes);
	ei.nParam4 = GetU32(pUserRate->nDruationTime);
...
void CEventDecode::PushParam_APPOpenInfo(unsigned int nIndex, unsigned int nTime, unsigned char *pData)
{
	...
	if (g_psTranslate->TransLonLat(fLat, fLon))
	{
		ei.nParam1 = fLat*1000000;
		ei.nParam2 = fLon*1000000;
	}else
	{
		ei.nParam1 = GetFLOAT(pOpenAppInfo->lat)*1000000;
		ei.nParam2 = GetFLOAT(pOpenAppInfo->lon)*1000000;
	}
	ei.nParam1 = nTime;
	wchar_t szBuffer[260] = {0};
	int nIndex_1 = 0;
...
//add 131203 wangyang
void CEventDecode::PushParam_APPOpenInfo_V2(unsigned int nIndex, unsigned int nTime, unsigned char *pData)
{
	if(NULL == pData)
	{
		return;
	}
	TEVENT_H_APP_OPEN_STR_V2* pOpenAppInfo_V2 = (TEVENT_H_APP_OPEN_STR_V2 *)pData;
	T_PARAM_EVENT_ITEM ei;
	InitEventItem(&ei, nIndex, nTime, TEVENT_ID_APP_OPEN_STATS_V2, 0, 0);
	//strcpy(ei.szEventName, pOpenAppInfo_V2->appID);
	double fLat = GetFLOAT(pOpenAppInfo_V2->lat);
	double fLon = GetFLOAT(pOpenAppInfo_V2->lon);
	if (g_psTranslate->TransLonLat(fLat, fLon))
	{
		ei.nParam1 = fLat*1000000;
		ei.nParam2 = fLon*1000000;
	...
	{
		ei.nParam1 = GetFLOAT(pOpenAppInfo_V2->lat)*1000000;
		ei.nParam2 = GetFLOAT(pOpenAppInfo_V2->lon)*1000000;
	}
	ei.nParam1 = nTime;
	//begin :appID change into elongate type (httpUrl改为变长)
	int nIdLen=pOpenAppInfo_V2->uAppIDLen;
	if(nIdLen >= sizeof(ei.szEventName))
	{
		return;
	}
	memcpy(&(ei.szEventName), (const char*)pOpenAppInfo_V2->uApp, nIdLen);
	//end
	const int nNameLen = pOpenAppInfo_V2->uAppNameLen;
	wchar_t szBuffer[256] = {0};
	int nIndex_1;
	for (int n=0,nIndex_1=0; n<nNameLen;n += 2,nIndex_1++)
	{
		memcpy(&(szBuffer[nIndex_1]), &(pOpenAppInfo_V2->uApp[nIdLen + n]), 2);
	}
	_gConverW2M(ei.szRemark,  szBuffer, nNameLen, sizeof(ei.szRemark));
	PushEventMatrix(&ei);
	return;
}
//add end
...
void CEventDecode::PushParam_ComplanitAPPInfo(unsigned int nIndex, unsigned int nTime, unsigned char *pData)
{
	...
	ei.nParam3 = GetI32(pHAppinfo->gsmDuration);
	ei.nParam4 = GetI32(pHAppinfo->wcdmaDuration);
	ei.iParam5 = GetI32(pHAppinfo->wifiDuration);
	
	ei.iParam6 = GetI32(pHAppinfo->gsmBytes);
	ei.iParam7 = GetI32(pHAppinfo->wcdmaBytes);
	ei.iParam8 = GetI32(pHAppinfo->wifiBytes);
	ei.iParam9 = GetI32(pHAppinfo->useTimes);
	wchar_t szBuffer[260] = {0};
	int nIndex_1 = 0;
...
//add 131203 wangyang
void CEventDecode::PushParam_ComplanitAPPInfo_V2(unsigned int nIndex, unsigned int nTime, unsigned char *pData)
{
	if(NULL == pData)
	{
		return;
	}
	TEVENT_H_APP_INFO_STR_V2 *pHAppinfo_V2 = (TEVENT_H_APP_INFO_STR_V2 *)pData;
	T_PARAM_EVENT_ITEM ei;
	InitEventItem(&ei, nIndex, nTime, TEVENT_ID_APPINFO_V2, 0, 0);
	//strcpy(ei.szEventName, pHAppinfo->appID);
	ei.nParam1 = GetI32(pHAppinfo_V2->activeDuration);
	ei.nParam2 = GetI32(pHAppinfo_V2->onlineDuration);

	ei.nParam3 = GetI32(pHAppinfo_V2->gsmDuration);
	ei.nParam4 = GetI32(pHAppinfo_V2->wcdmaDuration);
	ei.iParam5 = GetI32(pHAppinfo_V2->wifiDuration);
	
	ei.iParam6 = GetI32(pHAppinfo_V2->gsmBytes);
	ei.iParam7 = GetI32(pHAppinfo_V2->wcdmaBytes);
	ei.iParam8 = GetI32(pHAppinfo_V2->wifiBytes);
	ei.iParam9 = GetI32(pHAppinfo_V2->useTimes);
	//begin :appID change into elongate type (httpUrl改为变长)
	int nIdLen = pHAppinfo_V2->uAppIDLen;
	if(nIdLen >= sizeof(ei.szEventName))
	{
		return;
	}
	memcpy(&(ei.szEventName), (const char*)pHAppinfo_V2->uApp, nIdLen);
	//end
	wchar_t szBuffer[260] = {0};
	int nIndex_1;
	const int nNameLen = pHAppinfo_V2->uAppNameLen;
	for (int n=0,nIndex_1=0; n<nNameLen; n += 2,nIndex_1++)
	{
		memcpy(&(szBuffer[nIndex_1]), &(pHAppinfo_V2->uApp[nIdLen + n]), 2);
	}
	_gConverW2M(ei.szRemark,  szBuffer, nNameLen, sizeof(ei.szRemark));
	PushEventMatrix(&ei);
	return;
...
//add 131203 wangyang
void CEventDecode::PushPara_ComplanitHTTP_V2(unsigned int nIndex, unsigned int nTime, unsigned char *pData)
{
	if(NULL == pData)
	{
		return;
	}
	TEVENT_H_HTTP_URL_STR_V2* pHttpURL_V2 = (TEVENT_H_HTTP_URL_STR_V2*)pData;
	T_PARAM_EVENT_ITEM ei;
	InitEventItem(&ei, nIndex, nTime, TEVENT_ID_HTTP_URL_V2, 0, 0);
	ei.nParam1 = GetI32(pHttpURL_V2->accessTimes);
	//begin :httpUrl change into elongate type (httpUrl改为变长)
	//strcpy(ei.szEventName, pHttpURL->httpUrl);
	int nUrlLen = pHttpURL_V2->uHttpUrlLen;
	if(nUrlLen >= sizeof(ei.szEventName))
	{
		return;
	}
	memcpy(&(ei.szEventName), (const char*)pHttpURL_V2->uURL, nUrlLen);
	//end
	wchar_t szBuffer[256] = {0};
	int nIndex_1;
	const int nNameLen = pHttpURL_V2->uURLNameLen;
	for (int n=0,nIndex_1=0; n<nNameLen; n += 2,nIndex_1++)
	{
		memcpy(&(szBuffer[nIndex_1]), &(pHttpURL_V2->uURL[nUrlLen + n]), 2);
	}
	_gConverW2M(ei.szRemark, szBuffer, nNameLen, sizeof(ei.szRemark));
	PushEventMatrix(&ei);
	return;
...
void CEventDecode::PushParam_CAD(unsigned int nIndex, unsigned int nTime, unsigned char *pData)
{
	...
	ei.nSystem = p->System ;
	ei.nParam3 = p->CallcontextID;
	ei.nParam4 = GetI32(p->CSdisccause);
...
int CEventDecode::GetParamValue_int(unsigned int nParamID,   unsigned int nTimeIndex, bool bUnkpi)
{
	if (m_pParamList[nParamID].nDataType != E_PARAM_TYPE_INT)
	{
...
float CEventDecode::GetParamValue_float(unsigned int nParamID, unsigned int nTimeIndex, bool bUnkpi)
{
	if (m_pParamList[nParamID].nDataType != E_PARAM_TYPE_FLOAT)
	{
		return INVALID_VALUE;
...
void CEventDecode::PushParam(unsigned int nIndex, unsigned int nTime, unsigned int nParamID, unsigned char *pData)
{
	...
	//add 131203 wangyang
	else if(TEVENT_ID_APPINFO_V2 == nParamID)
	{
		PushParam_ComplanitAPPInfo_V2(nIndex, nTime, pData);
	}
	else if(TEVENT_ID_HTTP_URL_V2 == nParamID)
	{
		PushPara_ComplanitHTTP_V2(nIndex, nTime, pData);
	}
	else if(TEVENT_ID_APP_OPEN_STATS_V2 == nParamID)
	{
		PushParam_APPOpenInfo_V2(nIndex, nTime, pData);
	}
	//add end
	else if (TEVENT_ID_OFF_NET == nParamID)
	{
		PushParam_OffNet(nIndex, nTime, pData);
	}else if (TEVENT_ID_USERINFORATE == nParamID)
	{
		PushParam_UserRate(nIndex, nTime, pData);
	}else if(TEVENT_ID_USERINFORATE_APP == nParamID)
	{
		PushParam_UserAppRate(nIndex, nTime, pData);
	}
}
